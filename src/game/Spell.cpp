...
void Spell::cast(bool skipCheck)
{
    ...
    if (m_spellInfo->Id == 55610)
        if(const SpellEntry* spellInfo = sSpellStore.LookupEntry(m_spellInfo->Id))
            const_cast<SpellEntry*>(spellInfo)->AttributesEx6 |= SPELL_ATTR_EX6_NO_STACK_BUFF;
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            if (m_spellInfo->Id == 7744)                // Will of the Forsaken
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (m_spellInfo->SpellIconID == 207 && (m_spellInfo->SpellFamilyFlags.test<CF_WARLOCK_VOIDWALKER_SPELLS>()))
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            {
                if (m_spellInfo->Id == 781 || m_caster->hasUnitState(UNIT_STAT_ROOT))
    ...
    switch(m_spellInfo->rangeIndex)
    {
        ...
        case SPELL_RANGE_IDX_COMBAT:
        {
            if (target)
            {
                ...
                float range_mod = strict ? 1.25f : 5.0f;
